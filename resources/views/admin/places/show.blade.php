...
<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
        <h1 class="text-lg font-semibold text-gray-800 dark:text-white">Détails de la place</h1>
        <div class="flex space-x-2">
            <a href="{{ route('admin.places.edit', $place) }}" class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 active:bg-yellow-800 focus:outline-none focus:border-yellow-800 focus:ring ring-yellow-300 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    @if(!$place->est_disponible)
                        @php
                            $reservationActive = $place->reservations()->where('est_active', true)->with('user')->first();
                        @endphp
                        ...
                        @if($reservationActive)
                            ...
                            <div class="mt-4">
                                <form action="{{ route('admin.attribution.terminer', $reservationActive) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                            ...
                            @foreach($historiqueReservations as $reservation)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.historique.show', $reservation) }}" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300">
                                            Détails
                                        </a>
                                    </td>
                                </tr>
